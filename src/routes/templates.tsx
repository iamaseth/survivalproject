...
function TemplatesPage() {
  ...
  const upsertM = useMutation({
    mutationFn: (input: {
      id?: string; name: string; segment: string | null; subject: string; body: string;
    }) => upsert({ data: input }),
    onSuccess: () => {
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {templates.map((t) => {
                ...
                const canDelete = isExec || (currentUserId && t.createdBy === currentUserId);
                return (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {canDelete ? (
                          <button
                            onClick={() => {
                              if (confirm(`Delete template "${t.name}"? This cannot be undone.`)) deleteM.mutate(t.id);
                            }}
                            disabled={deleteM.isPending}
                            className="inline-flex items-center gap-1 rounded-md border border-input px-2 py-1 text-xs text-red-700 hover:bg-red-50 disabled:opacity-60"
                          >
...
function TemplateEditor({
  ...
  onClose: () => void;
  onSave: (v: { id?: string; name: string; segment: string | null; subject: string; body: string }) => void;
  saving: boolean;
      ...
      >
        ...
        <div className="grid flex-1 grid-cols-1 gap-0 overflow-auto md:grid-cols-[1fr_1fr]">
          <div className="space-y-4 border-r border-border p-6">
            <Field label="Name">
              <input
                ...
                placeholder="e.g. Outdoor — Warm intro (v1)"
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              />
            </Field>
            <Field label="Segment" hint="Optional. Free-form (Pet, Fitness, Outdoor…). Leave blank for General.">
              <input
            ...
            <Field label="Subject">
              <input
                ...
                placeholder="Quick idea for {{creator_name}} × Survival Tabs"
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              />
            </Field>
            <Field label="Body">
            ...
            <div className="rounded-md border border-dashed border-border bg-secondary/30 p-3 text-xs">
              ...
              <ul className="space-y-1 text-muted-foreground">
                {MERGE_FIELD_HINTS.map((f) => (
                  <li key={f.key}>
                    <code className="rounded bg-background px-1.5 py-0.5 text-[11px]">{`{{${f.key}}}`}</code> — {f.label}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="space-y-4 p-6">
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">Preview with sample creator</div>
            ...
            {willUnapprove ? (
              <div className="flex items-start gap-2 rounded-md bg-amber-50 p-3 text-xs text-amber-900">
                ...
                <div>
                  This template is currently <strong>approved</strong>. Saving these edits will move it back to
                  <em> Needs re-approval</em>, and an executive or partnership manager must re-approve before it can be used for sends.
                </div>
              </div>
            ) : null}
            ...
            {initial ? (
              <div className="text-[11px] text-muted-foreground">
                Current status: <StatusPill status={status} />
              </div>
            ) : null}
          </div>
        </div>
